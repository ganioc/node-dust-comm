...
function getParam(ds, cb) {
  if (ds.CN === getDownlinkCNCode('PARAM_GETTIME_REQ')) {
    getParamTime(ds, cb)
  } else if (ds.CN === getDownlinkCNCode('PARAM_GETRTDATA_INTERVAL_REQ')) {
    getParamRTDataInterval(ds, cb)
  } else {
    throw new Error('Error: common.js getParam , unrecognized cn code:' + ds.CN);
  }
}

function getParamTime(ds, cb) {
  var obj = str2obj(ds.CP);
  var out = {};
  for (var key in obj) {
    switch (key) {
...
function getParamRTDataInterval(ds, cb) {
  var out = {
    RtdInterval: RTDATAINTERVAL
  };
  cb(out)
}
